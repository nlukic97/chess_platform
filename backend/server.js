...
/*     socket.emit('move-made',data) //sending to the person who submitted the move
  ...
  socket.on('message-sent',(msg)=>{
    ...
    if(room){
      socket.to(room.id).emit('message-received',{msg:msg, timestamp: Date().getTime()}) //sends the event to all users in the room except the sender (so, to the other player)
    }
  })
